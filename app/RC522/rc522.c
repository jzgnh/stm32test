...
#define rc522spi    bus_spi1

#define cs_reset()  rc522spi->cs(1)
#define cs_set()    rc522spi->cs(0);


static uint8_t SPI1SendByte(uint8_t data) {
    return ((uint8_t)spi_rw(rc522spi, (uint8_t)data));
}

static void SPI1_WriteReg(uint8_t address, uint8_t value) {
	cs_reset();
	spi_rw(rc522spi, address);
	spi_rw(rc522spi, value);
	cs_set();
}
...
uint8_t MFRC522_ToCard(uint8_t command, uint8_t* sendData, uint8_t sendLen, uint8_t* backData, uint16_t* backLen) {
	...
	uint32_t xi = HAL_GetTick() + 25;	// i according to the clock frequency adjustment, the operator M1 card maximum waiting time 25ms
	do {
		// CommIrqReg[7..0]
		// Set1 TxIRq RxIRq IdleIRq HiAlerIRq LoAlertIRq ErrIRq TimerIRq
		n = MFRC522_ReadRegister(MFRC522_REG_COMM_IRQ);
		i--;
	} while ((HAL_GetTick()<xi) && !(n&0x01) && !(n&waitIRq));
